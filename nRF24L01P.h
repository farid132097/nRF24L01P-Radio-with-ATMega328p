...
uint8_t RF_RX_TIMEOUT(rx_buf, get_rx_data_len, rx_timeout, rx_buf_flash)
...
#define  MCU_SS_DDR        DDRB   
#define  MCU_SS_PORT       PORTB  
#define  MCU_SS            2      
...
#define  TX                0x00
#define  NA                0x00
#define  CRC_LSBYTE_POS    31
#define  CRC_MSBYTE_POS    30
#define  LEN_BYTE_POS      29
#define  RX_ADDR_BYTE_POS  28
#define  OWN_ADDR_BYTE_POS 27
...
uint8_t RF_RX_TIMEOUT(uint8_t *buf, uint8_t *len, uint16_t timeout, uint8_t flash){
RF_PWR(RX);
uint8_t dummy[2];
if(flash){RF_RW_REG(0xE2,WRITE,dummy,0);}
...
while(rty<retry){
  ...
  if(RF_RX_TIMEOUT(rbuf,&temp_len,(ACK_WAIT_MS*10),0)){
